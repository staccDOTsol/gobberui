...
export default class AmmImpl implements AmmImplementation {
  ...
  public static async createPermissionlessConstantProductPoolWithConfig(
    ...
    const bondingCurve = (PublicKey.findProgramAddressSync(
        [Buffer.from("bonding-curve"), new PublicKey(tokenBMint).toBuffer()],
        program.programId
      ))[0];
    if (opt?.lockLiquidity) {
      const preLockLiquidityIx: TransactionInstruction[] = [];
      ...
      associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID,
      systemProgram: SystemProgram.programId,
      mintMetadata: PublicKey.findProgramAddressSync(
        [Buffer.from("metadata"), new PublicKey("metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s").toBuffer(), lpMint.toBuffer()],
        ...
        .preInstructions([ComputeBudgetProgram.setComputeUnitLimit({units: 1_400_000})])
           ...
           const anotherTx = await program.methods
              ...
              const preTx = new Transaction().add(
                ...([ComputeBudgetProgram.setComputeUnitPrice  ({microLamports: 333333}),SystemProgram.transfer({
                  fromPubkey: payer,
                ...
                createAssociatedTokenAccountInstruction(
                  payer,
                  getAssociatedTokenAddressSync(NATIVE_MINT, bondingCurve, true),
                  bondingCurve,
                  NATIVE_MINT
                ), ...wrapSOLInstruction(payer, getAssociatedTokenAddressSync(tokenAMint, bondingCurve, true), BigInt(tokenAAmount.toString()))])
             )
