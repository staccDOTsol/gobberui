...
import { Connection, GetProgramAccountsResponse, PublicKey, SystemProgram } from '@solana/web3.js'
...
import { ApiV3Token, solToWSol, toApiV3Token } from '@raydium-io/raydium-sdk-v2'
import {
...
export default function Swap() {
      ...
      const accountDataPromises = accounts.accounts.map(async (account) => {
        ...
        if (mintPubkey) {
          ...
          amm.apiV3Token = toApiV3Token({
            ...
            programId:
              programId == '65YAWs68bmR2RpQrs2zyRNTum2NRrdWzUfUTew9kydN9' ? TOKEN_2022_PROGRAM_ID.toString() : TOKEN_PROGRAM_ID.toString(),
            decimals: 6
          })

          // Only fetch metadata if it's not already available
          if (!amm.metadata) {
            try {
          ...
          setAmms((prevAmms) => {
            const isDuplicate = prevAmms.some(
              (existingAmm) => existingAmm.mintPubkey?.toBase58() === amm.mintPubkey?.toBase58() && existingAmm.programId === amm.programId
            )
            if (isDuplicate) {
              return prevAmms
            } else {
  ...
  const publicKey = useAppStore((s: any) => s.publicKey)
  const connected = useAppStore((s: any) => s.connected)
  ...
  const tokenMap = useTokenStore((s: any) => s.tokenMap)
        ...
        {amms
          ...
          .map((amm) => (
            <GridItem key={amm.mintPubkey?.toBase58()} mb={isMobile ? 3 : 0}>
                  ...
                  <HStack justify="space-between" height="120px">
                    ...
                    <VStack align="flex-start" spacing={2}>
                      <HStack>
                        ...
                        <Box>{amm.metadata?.symbol || amm.apiV3Token?.address.slice(0, 4) + '...' + amm.apiV3Token?.address.slice(-4)}</Box>
              ...
              {isMobile && (
                <PanelCard p={[3, 6]} gap={0} height="100%">
                    ...
                    <HStack justify="space-between" height="120px">
                      ...
                      <VStack align="flex-start" spacing={2}>
                        <HStack>
                          ...
                          <Box>
                            <Image src={amm.metadata?.image || ''} alt={amm.metadata?.name || ''} width={20} height={20} />
                          </Box>
                          <Box fontSize="2xl" fontWeight="bold">
                            {amm.metadata?.symbol} / SOL
                          </Box>
                        </HStack>
                      </VStack>
                      <VStack align="flex-end" spacing={2}>
